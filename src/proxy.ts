...
export async function proxy(req: NextRequest) {
  ...
  const token = await getToken({
    ...
    secret:
      ...
      "academic-planner-super-secret-production-key-2026",
  });
  ...
  const activeRole = token.activeRole as { name?: string } | undefined;
  const activeRoleName = activeRole?.name ?? "";
  const availableWorkspaces = Array.isArray(token.availableWorkspaces)
    ? token.availableWorkspaces
    : [];
