...
export const authService = {
  ...
  async login(payload: LoginRequest): Promise<LoginResponse> {
    const res = await fetch(`${GATEWAY_BASE_URL}/api/v1/auth/login`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      if (res.status === 401) {
        ...
        const detail =
          errorData?.data?.detail || errorData?.message || 'Authentication failed. Please check your credentials.';
        throw new AuthApiError(detail, 401, errorData?.data, errorData?.errors);
      }

      if (res.status === 422) {
        const errorData = json as LoginErrorResponse;
        const msg = errorData?.message || 'Validation failed. Please check the provided information.';
        throw new AuthApiError(msg, 422, errorData?.data, errorData?.errors);
      }

      const fallbackMsg = json?.message || `Login failed with status ${res.status}`;
  ...
  async refresh(payload: RefreshRequest): Promise<RefreshResponse> {
    const res = await fetch(`${GATEWAY_BASE_URL}/api/v1/auth/refresh`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json',
    ...
    if (!res.ok) {
      if (res.status === 401) {
        ...
        const detail =
          errorData?.data?.detail || errorData?.message || 'Invalid, expired, or revoked refresh token.';
        throw new AuthApiError(detail, 401, errorData?.data, errorData?.errors);
      }

      if (res.status === 422) {
        const errorData = json as RefreshErrorResponse;
        const msg = errorData?.message || 'Validation error while refreshing token.';
        throw new AuthApiError(msg, 422, errorData?.data, errorData?.errors);
      }

      const fallbackMsg = json?.message || `Token refresh failed with status ${res.status}`;
      throw new AuthApiError(fallbackMsg, res.status, json?.data, json?.errors);
    }
