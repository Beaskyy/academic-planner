...
export function ResourcesView() {
  ...
  const handlePrimaryAction = () => {
    // Only physical venues tab has the full modal in Figma aps-17-venue-edit
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-gray-900/90 backdrop-blur-md text-white px-4 py-2 rounded-full shadow-2xl flex items-center gap-3 z-50 text-xs border border-white/10">
        ...
        <div className="flex items-center gap-1">
          {(['physical', 'virtual', 'exclusive'] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => { setActiveTab(tab); setIsAddVenueOpen(false); }}
              className={`px-3 py-1 rounded-full font-semibold transition-all ${
                activeTab === tab && !isAddVenueOpen
                  ? 'bg-[#335cff] text-white shadow'
                  : 'text-gray-300 hover:text-white hover:bg-white/10'
              }`}
            >
              {tab === 'physical' ? 'Physical (aps-17)' : tab === 'virtual' ? 'Virtual (aps-17)' : 'Exclusive (aps-17)'}
            </button>
          ))}
          <button
            onClick={() => { setActiveTab('physical'); setIsAddVenueOpen(true); }}
            className={`px-3 py-1 rounded-full font-semibold transition-all ${
              isAddVenueOpen
                ? 'bg-[#335cff] text-white shadow'
                : 'text-gray-300 hover:text-white hover:bg-white/10'
