...
export const DASHBOARD_METADATA: Record<DashboardRoute, RoleRouteEntry> = {
  ...
  "/student-admin": {
    ...
    description:
      "Manage student records, enrolment, fees, and academic standing.",
    icon: "student-admin",
  },
  "/learning-admin": {
...
export function getRoleEntry(roleName: string): RoleRouteEntry {
  ...
  if (
    normalised.includes("student") &&
    (normalised.includes("admin") || normalised.includes("manage"))
  ) {
    return DASHBOARD_METADATA["/student-admin"];
  }
...
export function isRoleAllowedForRoute(
  ...
  if (
    ...
    pathname !== "/student-admin" &&
    userDashboard !== "/student"
  ) {
    return false;
  }
  if (
    pathname.startsWith("/student-admin") &&
    userDashboard !== "/student-admin"
  ) {
    return false;
  }
  if (
    pathname.startsWith("/learning-admin") &&
    userDashboard !== "/learning-admin"
  ) {
    return false;
  }
  ...
  const isAcademicRoute =
    pathname === "/" ||
    ACADEMIC_PLANNER_ROUTES.some((prefix) => pathname.startsWith(prefix));
...
/**
 * Returns all route prefixes that exist (for proxy pattern matching).
 */
